...
public class HSAuthentication {
	...
	private final com.subgraph.orchid.circuits.hs.HSDescriptorCookie cookie;
	
	public HSAuthentication(com.subgraph.orchid.circuits.hs.HSDescriptorCookie cookie) {
		this.cookie = cookie;
	}
	
	public byte[] decryptIntroductionPoints(byte[] content) throws
            com.subgraph.orchid.circuits.hs.HSAuthenticationException {
		...
		if(firstByte == 1) {
			...
			throw new com.subgraph.orchid.circuits.hs.HSAuthenticationException("Introduction points section begins with unrecognized byte ("+ firstByte +")");
		}
	}
	...
	private byte[] decryptIntroductionPointsWithBasicAuth(ByteBuffer buffer) throws
            com.subgraph.orchid.circuits.hs.HSAuthenticationException {
		if(cookie == null || cookie.getType() != com.subgraph.orchid.circuits.hs.HSDescriptorCookie.CookieType.COOKIE_BASIC) {
	...
	private byte[] findKeyInAuthEntries(List<BasicAuthEntry> entries, byte[] id) throws
            com.subgraph.orchid.circuits.hs.HSAuthenticationException {
		...
		throw new com.subgraph.orchid.circuits.hs.HSAuthenticationException("Could not find matching cookie id for basic authentication");
	}
	
	private byte[] decryptAuthEntry(BasicAuthEntry entry) throws
            com.subgraph.orchid.circuits.hs.HSAuthenticationException {
	...
	private byte[] decryptIntroductionPointsWithStealthAuth(ByteBuffer buffer) {
		if(cookie == null || cookie.getType() != com.subgraph.orchid.circuits.hs.HSDescriptorCookie.CookieType.COOKIE_STEALTH) {
