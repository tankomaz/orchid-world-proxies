...
import com.worldproxies.orchid.ExitCircuit;
import com.worldproxies.orchid.InternalCircuit;
...
import com.worldproxies.orchid.circuits.guards.EntryGuards;
import com.worldproxies.orchid.circuits.hs.HiddenServiceManager;
...
public class CircuitManagerImpl implements CircuitManager, DashboardRenderable {
	...
	private final ScheduledExecutorService scheduledExecutor = Threading
            .newSingleThreadScheduledPool("CircuitManager worker");
	...
	public CircuitManagerImpl(TorConfig config, TorClientConfig clientConfig, DirectoryDownloaderImpl directoryDownloader, Directory directory, ConnectionCache connectionCache, TorInitializationTracker initializationTracker) {
		this.config = config;
		this.directory = directory;
		this.connectionCache = connectionCache;
		this.pathChooser = CircuitPathChooser.create(directory, clientConfig);
		if(config.getUseEntryGuards() || config.getUseBridges()) {
			this.pathChooser.enableEntryGuards(new EntryGuards(config, clientConfig, connectionCache, directoryDownloader, directory));
		}
