package com.worldproxies.orchid.connections;

import com.worldproxies.orchid.Cell;
import com.worldproxies.orchid.Circuit;
import com.worldproxies.orchid.Connection;
import com.worldproxies.orchid.ConnectionFailedException;
import com.worldproxies.orchid.ConnectionHandshakeException;
import com.worldproxies.orchid.ConnectionIOException;
import com.worldproxies.orchid.ConnectionTimeoutException;
import com.worldproxies.orchid.Router;
import com.worldproxies.orchid.Threading;
import com.worldproxies.orchid.Tor;
import com.worldproxies.orchid.TorClientConfig;
import com.worldproxies.orchid.TorConfig;
import com.worldproxies.orchid.TorException;
import com.worldproxies.orchid.circuits.TorInitializationTracker;
import com.worldproxies.orchid.circuits.cells.CellImpl;
import com.worldproxies.orchid.crypto.TorRandom;
import com.worldproxies.orchid.dashboard.DashboardRenderable;
import com.worldproxies.orchid.dashboard.DashboardRenderer;
...
public class ConnectionImpl implements Connection, DashboardRenderable {
    ...
    private final TorClientConfig clientConfig;
//toro delete tor client, just for test
	public ConnectionImpl(TorConfig config, TorClientConfig clientConfig, SSLSocket socket, Router router, TorInitializationTracker tracker, boolean isDirectoryConnection) {
	...
	private void processRelayCell(Cell cell) {
		...
		try {
			...
			if(circuit == null) {
                if (clientConfig.getExitNodes() != null) {
                    logger.warning(
                            "Could not deliver relay cell for circuit id = " + cell.getCircuitId() + " on connection "
                                    + this + ". Circuit not found [" + clientConfig.getExitNodes());
                }
                    return;
                }
            } finally {
