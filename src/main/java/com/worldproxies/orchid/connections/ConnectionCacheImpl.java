...
public class ConnectionCacheImpl implements ConnectionCache, DashboardRenderable {
	...
	private class ConnectionTask implements Callable<ConnectionImpl> {
		...
		public ConnectionImpl call() throws Exception {
			...
			final ConnectionImpl conn = new ConnectionImpl(config, clientConfig, socket, router, initializationTracker, isDirectoryConnection);
			conn.connect();
			return conn;
		}
	}
    ...
    //todo delete clientconfig
    private TorClientConfig clientConfig;
	public ConnectionCacheImpl(TorConfig config, TorClientConfig clientConfig, TorInitializationTracker tracker) {
	...
	public Connection getConnectionTo(Router router, boolean isDirectoryConnection) throws InterruptedException,
            ConnectionTimeoutException, ConnectionFailedException, ConnectionHandshakeException {
		if(isClosed) {
