...
public class HSDescriptorParserTest {
	...
	public void testIntroductionPointParser() {
		final com.subgraph.orchid.circuits.hs.IntroductionPointParser parser = createIntroductionPointParserFromString(TEST_IPS[0]);
		DocumentParsingResult<com.subgraph.orchid.circuits.hs.IntroductionPoint> result = parser.parse();
		assertTrue(result.isOkay());
		final List<com.subgraph.orchid.circuits.hs.IntroductionPoint> ips = result.getParsedDocuments();
		assertEquals(2, ips.size());
		for(com.subgraph.orchid.circuits.hs.IntroductionPoint ip: result.getParsedDocuments()) {
			assertTrue(ip.isValidDocument());
		}
	...
	private com.subgraph.orchid.circuits.hs.HSDescriptorParser createDescriptorParserFromString(String s) {
		return new com.subgraph.orchid.circuits.hs.HSDescriptorParser(null, createFieldParser(s));
	}
	
	private com.subgraph.orchid.circuits.hs.IntroductionPointParser createIntroductionPointParserFromString(String s) {
		return new com.subgraph.orchid.circuits.hs.IntroductionPointParser(createFieldParser(s));
	}
